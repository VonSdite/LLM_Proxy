...
- `CodexProxyService`
  ...
  - 完成回调携带最终认证文件名和稳定账号 ID，并按实际响应模型优先计算请求发生时的 API 美元等价费用
...
- `ProviderRuntimeFactory`
  ...
  - Hook 加载期间临时加入项目根目录和 Hook 文件所在目录到 `sys.path`
- `ExecutorRegistry`
  - 负责 HTTP 上游连接
...
- `ProxyController`
  ...
  - Codex 完成回调同时写入最终认证文件名、账号 ID 和请求发生时的预计费用
...
- `CodexProxyService`
  ...
  - 使用内置日期化价格快照按响应模型优先估算单次 API 美元等价费用；未知模型或不完整 Token 用量不产生费用值
  - 当前价格快照日期为 `2026-09-14`，数据源为 `Wei-Shaw/model-price-repo` 的 `model_prices_and_context_window.json`
- `ClaudeProxyService`
  - 每次 Claude 数据面请求读取当前 `oauth.proxy_mode`、`oauth.proxy` 与 `oauth.verify_ssl`

### 3.3 Control-Plane Model Mapping Management

模型映射页在 `model_mapping.enabled=true` 时提供顶层 `模型映射` 导航项。页面按映射级启用状态展示“已启用”和“已禁用”表格，支持组内拖拽排序、复制、勾选导出、勾选批量删除和映射级启停。批量删除使用行内二次确认，并在确认后一次删除选中的映射。复制项插入到源映射下方，只复制映射定义，不复制当前目标、冷却和自动禁用等运行状态。弹窗维护映射 ID、目标选择策略、故障冷却秒数和目标列表；目标模型通过可搜索下拉选择，并可设置优先级、启用状态、拖拽顺序或删除。选择策略、优先级和故障冷却使用统一的说明卡片，支持悬停、点击、键盘焦点、关闭按钮和视口自动定位；选择策略说明两种策略，优先级说明排序和兜底规则，故障冷却说明临时冷却、自动禁用和请求级切换规则。人工禁用和自动禁用目标都显示“禁用”标签；人工禁用提示显示“手动禁用”，自动禁用提示展示最近一次失败的状态码和错误内容。重新启用并保存映射会清除该目标的运行故障状态。目标模型候选包含当前已启用 Provider 的运行时模型、Codex OAuth 文本和图片本地模型目录、Claude OAuth 本地模型目录；已禁用 Provider 的模型不进入候选。导入模型映射时保留当前目录中不存在的目标模型 ID，并在页面中标记为不可用。
...
已启用的映射 ID 可以与当前 Codex OAuth 文本、Codex OAuth 图片或 Claude OAuth 文本模型 ID 重复；创建和更新都允许使用同名 ID，数据平面按自定义映射路由。已禁用映射保留定义、目标顺序和模型权限记录，不参与路由或可用模型目录。`/v1/models` 将生效的同名条目标记为模型映射，并保留映射能力信息。底层目标从目录消失时标记为不可用，编辑器支持重新选择模型 ID 或删除目标；原 ID 保留期间维持原优先级和启停状态。弹窗中的目标行直接删除，编辑内容在保存后生效；整条映射的删除操作保留行内确认。

目标行的性能测试复用 `POST /api/providers/test-models`，展示可用性、首字延迟、TPS 和错误详情。测试使用所选 Provider 的配置、真实上游模型 ID 和 Auth Entry；Auth Group 默认选择首个启用条目，支持在行内调整测试凭据。该测试能力面向 Provider 目标；OAuth 目标使用对应调用接口验证。测试结果仅保留在当前编辑弹窗中，与映射的启停、冷却和自动禁用状态独立。更换模型 ID、删除目标行或关闭弹窗时清理测试状态，已失效请求的结果不回填到新目标。
...
```text
provider editor form snapshot / mapping target provider snapshot
  -> controller
  -> auth header resolve (api_key or auth_group + auth_entry)
  -> ProviderRuntimeFactory
  -> header_hook
...
OAuth Codex tab
  ...
  -> persist the first-use baseline and current / previous Codex 7-day window boundaries
  -> aggregate request count, tokens, actual saved cost, and full-window estimated value from request_logs
...
- 认证文件的人工禁用状态、最近配额、配额错误、数据面使用状态、最近成功认证文件以及当前和上一七天窗口基线保存在 `data/oauth/codex/.state/auth_files.json`
- `request_logs` 为 Codex 请求保存最终 `auth_file_name`、稳定 `auth_account_id` 和请求发生时的 `estimated_cost_usd`；历史价格变化不重算已保存费用
...
- OAuth 页面认证文件列表依次按已保存快照确认有额度、非禁用、`pro`、`prolite`、`plus`、`free`、其他套餐和认证文件名升序排列；每页最多展示 50 个，支持多文件导入、全选后批量启用、批量禁用、批量刷新额度和重置卡、卡片内选择并使用重置卡、ZIP 导出和批量归档删除
- 手动刷新或额度到期自动刷新时，同时查询 `wham/usage` 和 `wham/rate-limit-reset-credits`；页面加载不获取重置卡
- 重置卡下拉框在同一行显示名称和过期时间，选中态与展开项使用一致间距；控件宽度最大为 280px；过期卡由页面本地定时隐藏，不触发额外上游请求
