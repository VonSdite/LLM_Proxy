...
class ClaudeProxyService:
    ...
    def _proxy_with_candidate(
        ...
        *,
        ...
        self._sign_anthropic_messages_body(upstream_body)
    ...
    @classmethod
    def _sign_anthropic_messages_body(cls, body: dict[str, Any]) -> None:
        """按 CPA 的 Claude OAuth 请求方式重新签名已有 billing header。"""
        ...
        unsigned_body = cls._json_body_bytes_for_requests(body)
        cch = f"{cls._xxhash64(unsigned_body, CLAUDE_CCH_SEED) & 0xFFFFF:05x}"
    ...
    @classmethod
    def _xxhash64(cls, data: bytes, seed: int = 0) -> int:
        ...
        if length >= 32:
            ...
            limit = length - 32
            while index <= limit:
                v1 = cls._xxhash64_round(v1, int.from_bytes(data[index : index + 8], "little"))
                index += 8
                v2 = cls._xxhash64_round(v2, int.from_bytes(data[index : index + 8], "little"))
                index += 8
                v3 = cls._xxhash64_round(v3, int.from_bytes(data[index : index + 8], "little"))
                index += 8
                v4 = cls._xxhash64_round(v4, int.from_bytes(data[index : index + 8], "little"))
                index += 8
            value = (
                cls._xxhash64_rotl(v1, 1)
                + cls._xxhash64_rotl(v2, 7)
                + cls._xxhash64_rotl(v3, 12)
                + cls._xxhash64_rotl(v4, 18)
            ) & _XXH64_MASK
            value = cls._xxhash64_merge_round(value, v1)
            value = cls._xxhash64_merge_round(value, v2)
            value = cls._xxhash64_merge_round(value, v3)
            value = cls._xxhash64_merge_round(value, v4)
        else:
        ...
        while index + 8 <= length:
            ...
            value ^= cls._xxhash64_round(0, lane)
            value = (cls._xxhash64_rotl(value, 27) * _XXH64_PRIME_1 + _XXH64_PRIME_4) & _XXH64_MASK
            index += 8
        if index + 4 <= length:
            value ^= (int.from_bytes(data[index : index + 4], "little") * _XXH64_PRIME_1) & _XXH64_MASK
            value = (cls._xxhash64_rotl(value, 23) * _XXH64_PRIME_2 + _XXH64_PRIME_3) & _XXH64_MASK
            index += 4
        while index < length:
            value ^= (data[index] * _XXH64_PRIME_5) & _XXH64_MASK
            value = (cls._xxhash64_rotl(value, 11) * _XXH64_PRIME_1) & _XXH64_MASK
            index += 1
    ...
    @classmethod
    def _xxhash64_round(cls, acc: int, lane: int) -> int:
        acc = (acc + lane * _XXH64_PRIME_2) & _XXH64_MASK
        acc = cls._xxhash64_rotl(acc, 31)
        return (acc * _XXH64_PRIME_1) & _XXH64_MASK

    @classmethod
    def _xxhash64_merge_round(cls, acc: int, value: int) -> int:
        acc ^= cls._xxhash64_round(0, value)
