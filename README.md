...
- 候选筛选会跳过人工禁用、认证失败、当前进程内配额冷却中、token 刷新失败或缺少 `access_token` 的文件
...
- 额度快照显示窗口耗尽时，认证文件使用已耗尽 Codex 窗口中最早的 `reset_at` 作为下一次重查时间；没有 `reset_at` 时保留已有冷却或回退到 60 秒
- 冷却只保存在当前进程内；时间到后下一次构建候选列表会自动解除，服务重启也会丢失冷却状态
