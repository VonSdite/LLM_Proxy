...
- 多下游暴露：同一个 Provider 可以同时配置多个 `target_formats`，即配一个API地址，支持claude code/opencode/codex/cherry-studio等直接使用
...
启动时如果发现旧配置字段 `target_format`，系统会自动把它规范化为 `target_formats` 并回写配置文件；这是兼容迁移逻辑，后续会删除。
...
当上游不是标准 OpenAI 格式时，优先通过 `source_format` 和 `target_formats` 使用内置协议转换；运行时会按当前路由解析出实际命中的 `target_format`。如果还需要补充 Cookie、自定义 Header、特殊字段，或者对成功响应做额外清洗，再用 Hook 在代理层补充这部分逻辑。
...
- `target_formats`：当前 Provider 对下游暴露的协议格式列表；默认 `['openai_chat']`
...
如何理解 `source_format` 和 `target_formats`：

- `source_format` 描述上游实际上接受什么协议
- `target_formats` 描述这个 Provider 允许下游通过哪些协议访问
- 实际请求进入时，controller 会根据当前 route family 从 `target_formats` 里解析出本次命中的单个 `target_format`
- 旧配置字段 `target_format` 会在启动时自动迁移为 `target_formats`；这是弃用兼容逻辑，后续会删除
- 同一个 Provider 里，`openai_responses` 和 `codex` 不能同时出现在 `target_formats` 中；它们都走 `/v1/responses`，但请求语义不同

例如：

- 上游和下游都用 Chat Completions：`openai_chat -> [openai_chat]`
- 上游是 Responses，下游继续暴露 Chat Completions：`openai_responses -> [openai_chat]`
- 同一个上游同时暴露 Chat 和 Claude：`openai_chat -> [openai_chat, claude_chat]`
...
- 按 `target_formats` 校验当前路由是否匹配，并解析出本次请求实际使用的 `target_format`
- 支持流式和非流式响应
- 自动识别 SSE、NDJSON、WebSocket JSON 等上游返回形态
- `GET /v1/models` 会返回当前已启用 Provider 的模型列表，以及 `provider_name`、`source_format`、`target_formats`、`transport` 等元信息
...
- Provider 表单支持逐行测试模型可用性、首字延迟与 TPS
- Provider 表单支持多选下游格式，新建时默认选中 `openai_chat`、`codex`、`claude_chat`，并在前端自动处理互斥项替换
- Auth Group 增删改查
...
- “拉取模型” 属于控制平面预览请求；当 Provider 绑定 `auth_group` 时，会使用该组的第一个 entry 发起请求
- “测试模型” 也属于控制平面请求，但会要求显式选择一个 `Auth Entry`
...
- 会复用当前 Provider 的 `source_format`、`transport`、`proxy`、`timeout_seconds`、`verify_ssl` 和 Hook 配置
...
- 标准协议转换由 `source_format`、当前请求命中的 `target_format` 对应的 translator 完成
...
- `provider_target_format`：当前请求实际命中的下游格式
- `transport`
- `stream`
- `auth_group_name`
- `auth_entry_id`
...
- 新增 Provider 时，先用后台“拉取模型”确认接口可达，再用“测试”确认目标模型可用性后再保存 `model_list`
