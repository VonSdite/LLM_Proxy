...
LLM_Proxy 把模型服务、鉴权方式、协议转换和后台运维收敛到同一个 Flask + gevent 服务中，对下游提供 OpenAI Chat Completions、OpenAI Responses 和 Claude Messages 兼容接口。它既支持常规 Provider 代理，也支持 Codex OAuth 账号文件代理，并内置 Provider 管理、Auth Group 凭据池、白名单控制、日志统计、请求调试和 Hook 扩展能力。
...
- Codex OAuth 代理：内置 Codex 登录、认证文件管理、模型目录维护、配额查看、账号状态记录和账号候选切换
...
- `/oauth`：Codex OAuth 认证文件与模型管理；开启 `oauth.enabled` 后会显示在后台导航中
...
| 路由 | 说明 | 下游协议 | 支持模型来源 |
| --- | --- | --- | --- |
| `POST /v1/chat/completions` | OpenAI Chat Completions 兼容接口 | `openai_chat` | Provider、Codex OAuth |
| `POST /v1/responses` | OpenAI Responses / Codex 风格接口 | `openai_responses` | Provider、Codex OAuth |
| `POST /v1/messages` | Claude Messages 兼容接口 | `claude_chat` | Provider、Codex OAuth |
| `GET /v1/models` | 返回当前已注册模型列表 | 不适用 | Provider、Codex OAuth |
...
Codex OAuth 模型是一个例外：模型 ID 直接使用 OAuth 模型目录里的裸模型名，例如 `gpt-5.4`。只有在存在 Codex 认证文件，并且该模型已添加到 `/oauth` 页面模型目录后，它才会出现在 `GET /v1/models` 中。
...
- `oauth.proxy`：OAuth 登录、配额查询和 Codex 上游请求使用的代理地址，留空表示不使用代理
- `oauth.verify_ssl`：OAuth/Codex 网络请求是否校验证书，默认 `false`
...
Codex OAuth 代理是独立于常规 Provider 的运行时能力。它不需要在 `providers[]` 中配置 API Key，而是通过后台 OAuth 登录生成本地认证文件，再把下游 OpenAI/Claude 兼容请求转换后转发到 Codex Responses 上游。
...
开启后，后台导航会显示 `/oauth` 页面。`proxy` 和 `verify_ssl` 会同时影响 OAuth 登录换 token、配额查询和 Codex 上游代理请求。
...
Codex OAuth 模型需要在 `/oauth` 页面手动添加。添加后，如果至少有一个 Codex 认证文件，`GET /v1/models` 会返回这些模型，并标记：
...
- `GET /v1/models` 会返回当前已启用 Provider 和 Codex OAuth 模型列表，以及 `provider_name`、`source_format` 等元信息
...
- Codex OAuth 场景先在 `/settings` 开启 OAuth，再到 `/oauth` 完成登录、添加模型并刷新配额
- Codex OAuth 认证文件属于敏感文件，只应保存在受控机器本地
