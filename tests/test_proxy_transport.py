...
class ProviderTemplateTransportTests(unittest.TestCase):
    ...
    def test_oauth_template_contains_oauth_workflows(self) -> None:
        ...
        self.assertIn('id="codexDisableSelectedAuthFilesBtn"', html)
        self.assertIn('id="codexEnableSelectedAuthFilesBtn"', html)
        codex_refresh_index = html.index('id="codexRefreshSelectedQuotaBtn"')
        codex_disable_index = html.index('id="codexDisableSelectedAuthFilesBtn"')
        codex_enable_index = html.index('id="codexEnableSelectedAuthFilesBtn"')
        codex_import_index = html.index('id="codexImportAuthFilesBtn"')
        codex_export_index = html.index('id="codexExportSelectedAuthFilesBtn"')
        self.assertLess(codex_refresh_index, codex_disable_index)
        self.assertLess(codex_disable_index, codex_enable_index)
        ...
        self.assertIn('<path d="M9.5 8.5v7"></path>', html)
        self.assertIn('<path d="M14.5 8.5v7"></path>', html)
        self.assertIn('<path d="m10 8 5 4-5 4z"></path>', html)
        ...
        self.assertIn("function disableSelectedCodexAuthFiles", html)
        self.assertIn("function enableSelectedCodexAuthFiles", html)
        self.assertIn("function setSelectedOAuthAuthFilesEnabled", html)
        self.assertNotIn("showMessage(`正在${actionLabel}", html)
        self.assertIn("function triggerCodexAuthFileImport", html)
        self.assertIn("function handleCodexAuthFileImportChange", html)
        self.assertIn("function getCodexImportedAuthFileNames", html)
        self.assertIn("function refreshCodexAuthFileQuotas", html)
        ...
        self.assertEqual(2, html.count("{ key: 'disabled', label: '禁用', count: counts.disabled }"))
        self.assertNotIn("{ key: 'disabled', label: '已禁用', count: counts.disabled }", html)
        ...
        self.assertIn("if (!isOAuthAuthFileEnabled(file)) return 'disabled';", html)
        ...
        self.assertIn("await refreshCodexAuthFileQuotas([authFileName], { refreshResetCards: true });", html)
        self.assertIn("const importedNames = getCodexImportedAuthFileNames(result);", html)
        self.assertIn("await refreshCodexAuthFileQuotas(importedNames, { refreshResetCards: true });", html)
        self.assertIn("await refreshCodexAuthFileQuotas([name], { refreshResetCards: false });", html)
        ...
        self.assertIn('id="claudeAuthFileFilters"', html)
        self.assertIn('id="claudeSelectAllAuthFiles"', html)
        self.assertIn('id="claudeSelectedAuthFileCount"', html)
        self.assertIn('id="claudeDisableSelectedAuthFilesBtn"', html)
        self.assertIn('id="claudeEnableSelectedAuthFilesBtn"', html)
        claude_disable_index = html.index('id="claudeDisableSelectedAuthFilesBtn"')
        claude_enable_index = html.index('id="claudeEnableSelectedAuthFilesBtn"')
        claude_import_index = html.index('id="claudeImportAuthFilesBtn"')
        claude_export_index = html.index('id="claudeExportSelectedAuthFilesBtn"')
        self.assertLess(claude_disable_index, claude_enable_index)
        self.assertLess(claude_enable_index, claude_import_index)
        self.assertLess(claude_import_index, claude_export_index)
        ...
        self.assertIn("function setClaudeAuthFileEnabled", html)
        self.assertIn("function disableSelectedClaudeAuthFiles", html)
        self.assertIn("function enableSelectedClaudeAuthFiles", html)
        self.assertIn("function renderClaudeAuthFileFilters", html)
        self.assertIn("function getClaudeFilteredAuthFiles", html)
        self.assertIn("function getClaudeSelectedAuthFileNamesForCurrentFilter", html)
        self.assertIn("requestSetOAuthAuthFileEnabled('claude', name, enabled)", html)
        self.assertIn("oauth-auth-file-item${isEnabled ? '' : ' is-disabled'}", html)
...
"""
    ...
    def test_settings_template_contains_oauth_network_settings(self) -> None:
        ...
        self.assertIn("header 缺失或不是合法 IP 时，会<strong>回退到对端 IP</strong>", html)
        ...
        self.assertIn("它不保证读取操作系统桌面代理设置", html)
        self.assertIn("留空时等同直连", html)
        self.assertIn("支持在 URL 中填写账号密码，保存时会自动转义账号密码里的特殊字符", html)
        self.assertIn("用户名里包含冒号需要手动写成 <code>%3A</code>", html)
...
class DashboardTemplateTests(unittest.TestCase):
    ...
    def test_index_template_uses_lazy_loaded_tabs(self) -> None:
        ...
        self.assertIn("/static/css/index.css?v=20260905-1", index_html)
